...
VenueownerSchema.pre("save", async function (next) {
  ...
  VenueownerSchema.methods.generateJWT = async function () {
    return jwt.sign({ id: this._id }, process.env.JWT_SECRET, {
      expiresIn: "30d",
    });
  };
  
  VenueownerSchema.methods.comparePassword = async function (enteredPassword) {
    const isMatch = await bcrypt.compare(enteredPassword, this.password);
    return isMatch;
  };


const Venueowner = model("Venueowner", VenueownerSchema);
